    ...
    if (!state.rowing || !state.rowing.active || _homeDist < 300) {
      ...
      if (state.nations) {
        ...
        if (_owKey) {
          ...
          // Draw bot island using the REAL rendering pipeline (identical to player)
          if (_own.islandState) {
            // Swap full island state so drawWorldObjectsSorted works on bot data
            if (typeof swapToIsland === 'function') {
              swapToIsland(_own.islandState, botCX, botCY);
              state.faction = _owKey;
              // Ensure bot has required state arrays for rendering
              if (!state.pyramid) state.pyramid = { x: botCX, y: botCY - 40 };
              ...
              // Force ground items re-sort for bot island
              _groundSortFrame = -999;
              // Render everything using the same pipeline as player island
              if (typeof drawWorldObjectsSorted === 'function') drawWorldObjectsSorted();
              swapBack();
            }
          }
